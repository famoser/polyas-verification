...
When voting over the internet, the voter needs to be able to verify that their vote was cast correctly. This avoids trusting their voting device fully. This project implements voter-side verification (also called individual verification) for POLYAS 3.0 (Version 2.0, 24 July 2025). You can find the specification in the folder [spec](./spec); this folder is excluded from the MIT license (the copyright remains at POLYAS). Note that the POLYAS system does only publish part of their specification (and notably no code and no formal proofs of the full system).

This project has been developed for the [Gesellschaft für Informatik](https://gi.de/) (German). In collaboration with other researchers, a scientific publication documents the experience of implementing this verifier (and others), and formulates recommendations for similar projects ([paper](https://inria.hal.science/hal-04663997)). 
